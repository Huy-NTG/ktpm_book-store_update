...
import QuanLyBanSach.BUS.KhachHangBUS;
import QuanLyBanSach.DAO.MyConnect;
import QuanLyBanSach.DTO.KhachHang;
import QuanLyBanSach.DTO.NhanVien;
...
//	 */
	public QL_KhachHang() {
		...
		panel.add(lblNewLabel_1_4);
                ...
                JButton bt_clearData =  new JButton("Clear");
		bt_clearData.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				clearData();
			}
		});
		bt_clearData.setFont(new Font("Tahoma", Font.PLAIN, 20));
		bt_clearData.setBounds(572, 101, 153, 34);
		panel.add(bt_clearData);
                
		txt_ho = new JTextField();
		txt_ho.setFont(new Font("Tahoma", Font.PLAIN, 18));
		txt_ho.setColumns(10);
		txt_ho.setBounds(326, 57, 153, 34);
		panel.add(txt_ho);
		...
		txt_tongchitieu.setBounds(735, 58, 189, 28);
                txt_tongchitieu.setEditable(false);
		...
		btn_xuatPdf.setBounds(909, 22, 97, 28);
		panel_2.add(btn_xuatPdf);
	...
	private void themKH() {
		...
		boolean flag = KHBUS.themKhachHang(txt_ho.getText(), txt_ten.getText(), gioitinh);
		if (flag) {
			KHBUS.docDanhSach();
			loadingKhachHang();
		}
	}
        private void themKH1() {
		String gioitinh;
		if (gender == true) {
			gioitinh = "Nam";
		} else {
		...
		boolean flag = KHBUS.themKhachHang1(txt_maKH.getText(), txt_ho.getText(), txt_ten.getText(), gioitinh);
		if (flag) {
			KHBUS.docDanhSach();
			loadingKhachHang();
		}
	...
	private void xuLyXuatExcel() {
		...
		XuLyFileExcel xuatExcel = new XuLyFileExcel();
		xuatExcel.xuatPDF(tbl_KH,"Danh sách khách hàng");
	}
